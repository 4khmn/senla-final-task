...
public class ReviewRepository extends BaseRepository<Review, Long> {
    ...
    public List<Review> getBySellerWithPagination(String sellerUsername, Integer rating, String sortDirection, int page, int size) {
        ...
        Fetch<Review, Advertisement> adFetch = root.fetch("advertisement", JoinType.LEFT);
        adFetch.fetch("category", JoinType.LEFT);

        Fetch<Review, User> authorFetch = root.fetch("author", JoinType.LEFT);
        ...
        if (sortDirection != null && !sortDirection.isBlank()) {
            if ("asc".equalsIgnoreCase(sortDirection)) {
                orders.add(cb.asc(root.get("score")));
            } else if ("desc".equalsIgnoreCase(sortDirection)) {
                orders.add(cb.desc(root.get("score")));
            }
        }
    ...
    public List<Review> getByAuthorWithPagination(String authorUsername, int page, int size) {
        return entityManager.createQuery(
                        "SELECT r FROM Review r " +
                                "JOIN FETCH r.advertisement a " +
                                "JOIN FETCH a.category " +
                                "JOIN FETCH r.seller s " +
                                "JOIN FETCH s.profile " +
                                "WHERE r.author.username = :authorUsername " +
